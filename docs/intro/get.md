---
id: Getting Started
aliases: []
tags: []
sidebar_position: 1
---

### Group Training Preparation

#### Strength Session Preparation For Athletes

##### Equipment

| Essential    | Would be useful  | Valuable Investments |
|--------------|------------------|----------------------|
| Water        | Skip rope        | Sliders              |
| Foam Mat     | Resistance bands | Bar weight           |
| Towel        | Block            | Kettle bells         |
| Waterproofs  | Watch            | Rings                |
| Warm clothes |                  | Paralletts                     |


##### Periodisation (Draft) Winter Macro; Meso A (Week 48 to 52)

- Static Strength
- Technical strength skills
- Basic understanding
- Foundations
- Crawling basics
- Cross training; VO2 Max


##### Winter Macro Meso B

##### Winter Macro Meso C

AB
